...
const renderGallery = () => {
    for (let i = 0; i < 3; i++) { // Promise all
        ...
        imageToRender.then((path) => {
            const renderedImage = handleSuccessLoad(path, galleryImgWrapper);
...
btnGalleryPrev.addEventListener('click', () => {
    ...
    galleryImgWrapper.querySelectorAll('.gallery_img').forEach((imageDivElement, index) => {
